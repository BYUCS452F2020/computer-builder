...
{
	// MongoDB Build Service Functions--------------------------------------------------------------------------------
	...
	//		BuildDAO buildMDAO = new BuildDAO(database.openConnection());
			BuildMDAO buildMDAO = new BuildMDAO();

			List<Build> buildList = buildMDAO.findUserBuilds(username);
	...
	public SaveBuildResult insertNewBuild(SaveBuildRequest sbReq)
	{
		//Database database = new Database();
		...
		try
		{
			...
			buildMDAO.insertBuild(buildToAdd);
